<template>
  <div class="mt-8 sm:pl-4 md:pl-6 md:pt-8 lg:pl-8 sm:pb-16 sm:border-l border-ui-border md:mt-0">
    <slot></slot>
    <h3 class="pt-0 mt-4 text-sm tracking-wide uppercase border-none">Ir para</h3>
    <div class="mt-3">
      <ul>
        <li
          ...
          :class="{
            'pt-2 mt-2': index > 0 && heading.depth === 2,
        ...
        >
          <g-link
            ...
            class="relative flex items-center py-1 text-sm transition transform hover:translate-x-1 text-primary"
            :class="{
              'pl-2': heading.depth === 3,
              'pl-3': heading.depth === 4,
              'pl-4': heading.depth === 5,
              'pl-5': heading.depth === 6,
              'font-bold text-copy-primary': activeAnchor === heading.anchor
            }"
          >
            <span
              class="absolute w-2 h-2 -ml-3 rounded-full opacity-0 bg-blue-600 transition transform scale-0 origin-center"
              :class="{
                'opacity-100 scale-100 duration-500': activeAnchor === heading.anchor
              }"
            ></span>
            {{ heading.value }}
          </g-link>
...
export default {
  ...
  computed: {
    page () {
      return this.$page.episodio;
    },
    headings () {
      return this.page.headings.filter(h => h.depth > 1 && h.depth <= 2);
    }
  },
  watch: {
    $route: function () {
